...
function RoleBadge({ role }: { role: AppRole }) {
  return (
    <div className="px-4 py-3 bg-slate-950/30 border-b border-slate-800/80">
      <div className="flex items-center gap-2.5 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5">
        <span className="p-1 rounded-md bg-slate-700/60 border border-slate-600/50">
          <ShieldCheck className="w-3.5 h-3.5 text-pink-400" />
        </span>
        <span className="text-xs font-semibold text-white tracking-wide">{role}</span>
        <span className="ml-auto text-[10px] font-medium text-slate-400 uppercase tracking-widest">
          Role
        </span>
      </div>
    </div>
